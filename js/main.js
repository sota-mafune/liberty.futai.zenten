...
async function fetchByCOQL() {
    ...
    while(hasMore) {
        ...
        // ★ 診断フェーズ：一旦 WHERE 条件から nousyayoteibi を外して「計上日ベース」だけで取得できるか試します
        ...
        try { 
            ...
            if (res.data) { 
                ...
                // エラー内容を画面に大きく表示
                loadingEl.innerHTML = "<div style='color:red; background:#fff; padding:10px; border:2px solid red;'><h3>❌ CRMデータ取得エラー</h3>項目名が間違っている可能性があります。<br>コード: " + res.code + "<br>詳細: " + res.message + "</div>";
                hasMore = false; return;
            } else { hasMore = false; }
        } catch (e) { 
            console.error("COQL例外:", e);
            hasMore = false; 
        }
    } 
        ...
        if(budgetDataGlobal.daily_budget) {
            budgetDataGlobal.daily_budget.forEach(b => {
                ...
                if(d.startsWith(targetDateStr) && d <= endDateStr) {
                    ...
                    gS[gr].budget_current += parseInt(b["成約台数予算"])||0; storeStatsMaster[st].budget_current += parseInt(b["成約台数予算"])||0; totalS.budget_current += parseInt(b["成約台数予算"])||0;
                }
            });
        }
    }
...
function buildTable(sum, title, totalS) {
    ...
    const rowDef = [
        ...
        { sec: "基本実績" }, { lbl: "実績", m: "j", cls: "#ffe599" }, { lbl: "達成率", type: "total_ratio", n: "j", d: "budget_j", cls: "#ffffff" }, { lbl: "昨年実績(当日)", m: "empty", cls: "#d9d2e9" }, { lbl: "昨年対比", m: "empty", cls: "#d9d2e9" },
        { lbl: "新規接客数", m: "v_n", cls: "#ffffff" }, { lbl: "商談件数", m: "sho", cls: "#ffffff" }, { lbl: "商談率", type: "ratio", n: "sho", d: "v_n", cls: "#ffe599" }, { lbl: "成約率", type: "ratio", n: "j", d: "v_n", cls: "#ffe599", redText: true },
        { sec: "受注時想定" }, { lbl: "受注台数", type: "arari_val", val: "j", cls: "#ead1dc" }, { lbl: "@全部割(保証込)", type: "arari_avg", val: "ar25", cls: "#ead1dc" }, { lbl: "総粗利", type: "arari_sum", val: "ar25", cls: "#ead1dc" },
        { sec: "納車着地粗利予測" }, { lbl: "納車台数", type: "del_arari_val", cls: "#d9ead3" }, { lbl: "@全部割(保証込)", type: "del_arari_avg", val: "del_ar25", cls: "#d9ead3" }, { lbl: "総粗利(保証込)", type: "del_arari_sum", val: "del_ar25", cls: "#d9ead3" }, { lbl: "納車台数予算", m: "budget_del", type: "total_only", cls: "#d9ead3" }, { lbl: "総粗利保証込予算", m: "budget_arari", type: "total_only_money", cls: "#d9ead3" }, { lbl: "粗利達成率", type: "total_ratio", n: "del_ar25", d: "budget_arari", cls: "#d9ead3" }
    ];
    for(var j=0; j<rowDef.length; j++){ var r = rowDef[j]; if(r.sec) { h += "<tr><td class='sticky-col-item section-row' style='position: sticky !important; z-index: 180 !important; left: 0; border-right: none;'>" + r.sec + "</td><td class='sticky-col-total section-row' style='position: sticky !important; z-index: 170 !important; left: 170px; border-left: none;'></td><td colspan='" + keys.length + "' class='section-row'></td></tr>"; } else { h += "<tr><td class='sticky-col-item' style='background-color:"+(r.cls||"#fff")+"'>"+r.lbl+"</td>"; h += renderCell(totalS, r, true); for(var k=0; k<keys.length; k++) h += renderCell(sum[keys[k]], r, false); h += "</tr>"; } }
    h += "</tbody></table>"; return h;
}

function renderCell(s, r, isT) {
    var kVal = "-", fVal = "-", tVal = "-", bg = r.cls || "#ffffff", textClass = r.redText ? "force-red" : ""; var c = isT ? "sticky-col-total " : "";
    if(r.type === "total_only") { tVal = (s[r.m] || 0).toLocaleString(); return "<td class='"+c+"' style='background-color:"+bg+"'><div class='cell-stack' style='align-items:center; font-weight:bold; font-size:12px;'>" + tVal + "</div></td>"; }
    else if(r.type === "total_only_money") { tVal = (s[r.m] || 0).toLocaleString(); return "<td class='"+c+"' style='background-color:"+bg+"'><div class='cell-stack' style='align-items:center; font-weight:bold; font-size:12px;'>¥" + tVal + "</div></td>"; }
    else if(r.type === "total_ratio") { var actual = (s[r.n+"_k"] || 0) + (s[r.n+"_f"] || 0); var budget = s[r.d] || 0; tVal = budget > 0 ? Math.round((actual / budget) * 100) + "%" : "0%"; var ratioColor = (budget > 0 && (actual / budget) >= 1) ? "color: #d32f2f;" : "color: #333;"; return "<td class='"+c+"' style='background-color:"+bg+"'><div class='cell-stack' style='align-items:center; font-weight:bold; font-size:12px; " + ratioColor + "'>" + tVal + "</div></td>"; }
    ...
    else if(r.type && (r.type.startsWith("arari") || r.type.startsWith("del_arari"))) {
        var prefix = r.type.startsWith("del_arari") ? "del_" : "";
        if(r.type.endsWith("_val")) { kVal = s[prefix+"cnt_k"] || 0; fVal = s[prefix+"cnt_f"] || 0; tVal = kVal + fVal; }
        else if(r.type.endsWith("_avg")) { var den = (s[prefix+"cnt_k"] + s[prefix+"cnt_f"]) || 1; tVal = Math.round((s[r.val+"_k"] + s[r.val+"_f"]) / den).toLocaleString(); kVal = s[prefix+"cnt_k"] ? Math.round(s[r.val+"_k"]/s[prefix+"cnt_k"]).toLocaleString() : 0; fVal = s[prefix+"cnt_f"] ? Math.round(s[r.val+"_f"]/s[prefix+"cnt_f"]).toLocaleString() : 0; }
        else { tVal = Math.round(s[r.val+"_k"] + s[r.val+"_f"]).toLocaleString(); kVal = Math.round(s[r.val+"_k"]).toLocaleString(); fVal = Math.round(s[r.val+"_f"]).toLocaleString(); }
    ...
    else if(r.lbl === "実績") { kVal = s.j_k; fVal = s.j_f; tVal = kVal + fVal; return "<td class='"+c+"' style='background-color:"+bg+"'><div class='cell-stack'><div class='stack-label-3'><div style='width:50%;'>軽</div><div style='width:50%;'>普</div></div><div class='stack-values-3'><div class='val-kei'>"+kVal+"</div><div class='val-fu'>"+fVal+"</div></div><div class='stack-total-3'>"+tVal+"</div></div></td>"; }
    else {
